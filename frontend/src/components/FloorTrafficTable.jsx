...
export default function FloorTrafficTable({ rows, onEdit, onToggle }) {
  ...
  // Table headers (except Visit Time & Time on Lot, which are always first)
  const headers = [
      ...
      <tr key={row.id} className={rowClasses} onClick={() => handleRowClick(row.id)}>
        ...
        <td className="p-2 space-x-1" role="cell">
          <button
            ...
            onClick={e => { e.stopPropagation(); window.location.href = `tel:${row.phone ?? ''}`; }}>
            <Phone className="w-4 h-4" />
          </button>
          <button
            aria-label="Text"
            className="rounded-full p-2 hover:bg-gray-100 transition"
            onClick={e => { e.stopPropagation(); window.location.href = `sms:${row.phone ?? ''}`; }}>
            <MessageCircle className="w-4 h-4" />
          </button>
          <button
            aria-label="Email"
            className="rounded-full p-2 hover:bg-gray-100 transition"
            onClick={e => { e.stopPropagation(); window.location.href = `mailto:${row.email ?? ''}`; }}>
